...
text(s, IN(0.62), IN(1.55), IN(7.1), IN(0.9),
     [{"t": "「微信・LINE で完結する社内研修アシスタント」を、サーバーレス＋生成AIでフルスクラッチ開発。共通バックエンドをマルチチャネル展開し、受講管理・多言語対応・AI問答・自動リマインドを固定費ほぼゼロで実現しました。",
...
for t, col in pts:
    ...
    y += IN(0.62)
# 右：KPI 卡片
stats = [("6", "DynamoDB テーブル"), ("4", "Lambda 関数"), ("14", "自動テスト 全合格"),
         ("微信+LINE", "対応チャネル"), ("約0.5円", "AI 1問あたり(概算)"), ("日中", "多言語対応")]
...
text(s, IN(0.62), IN(1.55), IN(12), IN(0.7),
     [{"t": "BrightStar は、受講者が普段使う「微信(WeChat)」で完結する研修アシスタント。チャットだけで申込・確認ができ、講師は数行のコマンドで運営。Webサイトで成績・アンケートを記録します。",
       "sz": 14, "c": INK, "ls": 1.18}])
cols = [
    ("受講者", DEEP, ["微信/LINE 対応・言語選択→登録", "講座一覧・申込・キャンセル", "次回講座/Zoom 確認", "AI問答／アカウント連携"]),
...
row = IN(2.35)
node(IN(0.62), row, IN(2.25), IN(0.95), "微信/LINE/企業微信", "受講者・講師", NAVY)
...
node(IN(9.27), row, IN(3.45), IN(0.95), "Lambda (Python 3.12)", "webhook / line / web / reminder", TEAL)
...
text(s, IN(0.62), IN(5.45), IN(12), IN(1.4),
     [{"t": "設計の勘所", "sz": 13, "c": DEEP, "b": True, "f": HEAD, "sa": 4},
      ...
      {"t": "・ルーティング層をチャネル非依存に設計 → LINE は署名検証のみで同一バックエンドに接続(中継/暗号なし)、提醒は連携済み全チャネルへ配信", "sz": 12, "c": INK, "ls": 1.12}])
...
feats = [
    ...
    ("⏰", "自動リマインド", "開講1時間前に自動通知（微信・LINE 両対応）"),
