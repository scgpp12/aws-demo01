"""最小纯 Python AES（ECB block + CBC），支持 128/192/256 位密钥。

无第三方依赖（Lambda 无 Docker 编译 pycryptodome）。仅用于企业微信回调消息
的 AES-256-CBC 加解密；消息很短，纯 Python 性能足够。
正确性由 tests 里的 NIST FIPS-197 向量校验。
"""
...
class AES:
    ...
    def encrypt_block(self, block):
        return _encrypt_block(block, self._w, self._nr)

    def decrypt_block(self, block):
        return _decrypt_block(block, self._w, self._nr)
