...
# 6. 内容チェック（自動）
content_slide("提出時の内容チェック（自動）", "VALIDATION", [
    ("■ 拒否（保存しない）", 19, TEAL, True),
    "年月：経費 A1「YYYY年M月分」／勤怠 B5 の日付 が提出月と一致しなければ不可。",
    "氏名：勤怠 B3 の氏名が登録氏名と一致しなければ不可。",
    ("■ 警告（保存はする・勤怠）", 19, TEAL, True),
    "休日（土日・祝日）に勤務時間（6/7行）があれば該当日を挙げて確認を促す。",
    "5 行目の日付が 1 日〜月末まで揃っていなければ未入力日を通知。",
    "※ xlsx 解析は標準ライブラリのみ（依存ゼロ／ふりがな除外／日付シリアル対応）。",
], accent=GOLD)
